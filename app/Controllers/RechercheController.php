...
class RechercheController extends BaseController
{
    ...
    public function simpleSearch($request, $response, array $args)
    {
        ...
        if (!$results) {
            ...
            $results = $results == null ? [] : $results->with('type')->get();
            return $response->withJson($results);
        }

        return $response->withJson($results->with('type')->get());
    }
    ...
    public function advanceSearch($request, $response)
    {
        ...
        if (!empty($type)) {
            $resultats->where('document_type_id', $type);
        }
        if (!empty($lang)) {
        ...
        // renvoi les resultas
        $resultats = $resultats == null ? [] : $resultats->with('categorie')
                                                         ->with('type')
                                                         ->get();
